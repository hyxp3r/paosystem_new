...
class ExamWrite(ReportDataOperation):
    ...
    def prepare_data(self):
        ...
        case -- Сопоставление текстового названия экзамена в Тандеме с Уникальным названием курса в Мудле
            ...
            when D.TITLE_P = 'маг. Финансы и кредит' then 'Финансы и кредит ВИ'
        ...
        {f"AND {group_where} is not null" if self.request.get("group") == "Да" else ""}
...
class ExamMail(ReportDataOperation):
    ...
    def prepare_data(self):
        ...
        ,REPLACE(PContact.PHONEMOBILE_P, '+', '') 'Мобильный номер'
